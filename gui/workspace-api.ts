...
interface MinecraftSourcesModule {
  ...
  listProjectRuntimeMods(projectPath: string, loader: "fabric" | "forge" | "neoforge", mcVersion: string): Promise<unknown>;
}

interface ModIntelModule {
  searchExternalMods(request: unknown): Promise<unknown>;
  resolveExternalMod(request: unknown): Promise<unknown>;
  getModIntelStatus(): unknown;
  startModIntelSourceTask(request: unknown): unknown;
  readSourceReport(unitId: string): { path: string; content: string };
  sourceUnitPaths(unitId: string): { unitDir: string; sourceDir: string; reportPath?: string };
  listCompatibilityProfiles(projectPath: string, variantId?: string): unknown;
  upsertCompatibilityProfile(projectPath: string, variantId: string, input: unknown): Promise<unknown>;
}
...
let modIntelMod: ModIntelModule | null = null;
...
async function modIntel(): Promise<ModIntelModule> {
  if (!modIntelMod) {
    modIntelMod = await loadDist<ModIntelModule>(repoDist("mod-intel", "index.js"));
  }
  return modIntelMod;
}
...
export async function handleWorkspaceApi(
  ...
  if (compatMatch && (method === "POST" || method === "PATCH")) {
    ...
    try {
      const profile = await (await modIntel()).upsertCompatibilityProfile(
  ...
  if (runtimeModsMatch && method === "GET") {
    ...
    const sources = await minecraftSources();
    const mods = await sources.listProjectRuntimeMods(
      found.variant.projectPath,
      found.variant.loader,
      found.variant.mcVersion,
    );
    json(res, {
      rootPath: path.join(found.variant.projectPath, "run", "mods"),
      mods,
    });
    return true;
  }
  ...
  if (runtimeModsSourcesMatch && method === "POST") {
    ...
    const sources = await minecraftSources();
    const available = await sources.listProjectRuntimeMods(
      found.variant.projectPath,
      found.variant.loader,
      found.variant.mcVersion,
    ) as Array<{ file: string; supported: boolean }>;
    const allowed = new Set(available.filter((item) => item.supported).map((item) => path.resolve(item.file)));
  ...
  if (sourceMatch && method === "POST") {
    ...
    const body = (await readBody(req)) as { force?: boolean; includeDependencies?: boolean };
    try {
      const sources = await minecraftSources();
      const task = sources.startMinecraftSourceTask({
        scope: "single",
